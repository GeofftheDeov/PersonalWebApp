...
router.get("/characters", auth, async (req: any, res) => {
    try {
        ...
        const characters = await Character.find(query).populate("campaign dungeon").populate({ path: "player", select: "name firstName lastName email handle profilePicture" });
        res.json(characters);
    } catch (error: any) {
...
router.get("/characters/:id", auth, async (req: any, res) => {
    try {
        const character = await Character.findById(req.params.id).populate("campaign dungeon").populate({ path: "player", select: "name firstName lastName email handle profilePicture" });
...
router.put("/characters/:id", auth, async (req: any, res) => {
    try {
        ...
        const character = await Character.findByIdAndUpdate(
            ...
            { new: true }
        ).populate("campaign dungeon").populate({ path: "player", select: "name firstName lastName email handle profilePicture" });
        if (!character) return res.status(404).json({ error: "Character not found" });
        res.json(character);
    } catch (error: any) {
