...
if [ -n "$VAULT_TOKEN" ] && [ -n "$GITHUB_VAULT_REPO" ]; then
  VAULT_DIR="${OBSIDIAN_VAULT_PATH:-/obsidian-vault}"
  CLONE_URL="https://oauth2:${VAULT_TOKEN}@github.com/${GITHUB_VAULT_REPO}.git"
  ...
  echo "[vault] VAULT_TOKEN or GITHUB_VAULT_REPO not set — skipping vault sync"
fi
