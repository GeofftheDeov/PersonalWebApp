...
fix_script = f"""
const email = '{email}';
print('Deleting duplicate Account...');
const accResult = db.accounts.deleteMany({{email: email}});
print('Deleted ' + accResult.deletedCount + ' accounts');

print('Deleting duplicate Contact...');
const conResult = db.contacts.deleteMany({{email: email}});
print('Deleted ' + conResult.deletedCount + ' contacts');

print('Fixing User resetPasswordExpires type...');
const u = db.users.findOne({{email: email}});
if (u) {{
    if (typeof u.resetPasswordExpires === 'number') {{
        db.users.updateOne(
            {{_id: u._id}},
            {{$set: {{resetPasswordExpires: new Date(u.resetPasswordExpires)}}}}
        );
        print('Fixed User expiration type to Date.');
    }} else {{
        print('User expiration type is already ' + (typeof u.resetPasswordExpires) + ' - likely Date.');
    }}
}} else {{
...
args = [
    ...
    '--container', 'mongodb',
    '--interactive',
    '--command', f'mongosh personal_web_app --eval "{fix_script}"'
]
