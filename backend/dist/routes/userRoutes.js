...
router.post("/login", async (req, res) => {
    ...
    try {
        ...
        if (!user) {
            ...
            userType = "Lead";
            if (user)
                console.log(`[AUTH] Found match in Leads`);
        }
        if (!user) {
        ...
        // Generate Token
        const token = jwt.sign({ id: user._id, type: userType, email: user.email }, process.env.JWT_SECRET || "your-secret-key-change-this", { expiresIn: "1h" });
        res.json({
            token,
            user: {
                id: user._id,
...
router.post("/google-login", async (req, res) => {
    ...
    try {
        ...
        // Generate Token
        const jwtToken = jwt.sign({ id: user._id, type: userType, email: user.email }, process.env.JWT_SECRET || "your-secret-key-change-this", { expiresIn: "1h" });
        res.json({
            token: jwtToken,
            user: {
                id: user._id,
